    ...
    function tsp_hk_rec(cities, start) { // Recurvsive part of hk 
        if(cities.length == 1) { //If there is only one city left
            ...
            for(var i = 0; i < newCities.length; i++) { //Iterate through our list of new cities to find the shortest path
                var currentTourLength = tsp_hk_rec(newCities, newCities[i]) + distance_matrix[start][newCities[i]]; //Recursivly call our function to find the minimum length and then add the distance from the start to get the total length
